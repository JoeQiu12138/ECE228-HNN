#!/usr/bin/env python3
"""1D nonlinear oscillator HNN (Phase 1).

Usage (from src/Phase1/NLoscillator/):
    1. Set actF in odeNet_NLosc_MM (mySin, AdaptiveSin, Snake, GELU, Tanh, …).
    2. python3 HNN_NLoscillator.py

Artifacts: PNG under exp_data/1D/<activation>/; numerics archive under exp_data/1D/data/.
Checkpoints: models/ in this directory.
"""
...
from utils_NLoscillator import NLosc_exact, NLosc_solution, energy, saveData, symEuler

ROOT = Path(__file__).resolve().parent
REPO = ROOT.parents[2]  # repo root (src/Phase1/NLoscillator → … → ECE228-HNN)
EXPERIMENT_1D = REPO / "exp_data" / "1D"
DATA_DIR = EXPERIMENT_1D / "data"
MODELS_DIR = ROOT / "models"
DEFAULT_CKPT = MODELS_DIR / "model_NL.zip"


def experiment_dir_for(actF: torch.nn.Module) -> Path:
    """Map active activation to exp_data/1D/<folder>/ (Phase 2–style layout)."""
    sub = {
        "mySin": "baseline",
        ...
        "Tanh": "tanh",
        "SiLU": "SiLU",
    }.get(type(actF).__name__, "_scratch")
    out = EXPERIMENT_1D / sub
...
class Snake(torch.nn.Module):
    def __init__(self):
        ...
        # Learnable frequency a; the network tunes the effective oscillation scale
...
def run_odeNet_NLosc_MM(
    ...
    PATH = Path(PATH) if PATH is not None else DEFAULT_CKPT
...
## LOADING WEIGHTS PART if PATH file exists and loadWeights=True
    if PATH.exists() and loadWeights == True:
...
def trainModel(X0, t_max, neurons, epochs, n_train, lr,  loadWeights=False, minLoss=1e-5, showLoss=True):
    model, loss, runTime = run_odeNet_NLosc_MM(
        X0, t_max, neurons, epochs, n_train, lr, loadWeights=loadWeights, minLoss=minLoss
    )

    DATA_DIR.mkdir(parents=True, exist_ok=True)
    np.savetxt(DATA_DIR / "loss.txt", loss)
    out_dir = experiment_dir_for(model.actF)
    ...
    if showLoss == True:
        ...
        plt.ylabel("Loss")
        plt.xlabel("t")
        plt.savefig(out_dir / "nonlinearOscillator_loss.png")


def loadModel(PATH: str | Path | None = None):
    PATH = Path(PATH) if PATH is not None else DEFAULT_CKPT
    fc0 = odeNet_NLosc_MM(neurons)
    checkpoint = torch.load(PATH)
    fc0.load_state_dict(checkpoint['model_state_dict'])
...
OUT_DIR = experiment_dir_for(model.actF)
plt.savefig(OUT_DIR / "nonlinearOscillator_trajectories.png")
...
plt.savefig(OUT_DIR / "nonlinearOscillator_error.png")
...
# saveData(str(DATA_DIR) + '/', t_net, x, px, E)
# saveData(str(DATA_DIR / 'Euler100') + '/', t_s100, x_s100, p_s100, E_s100)
# np.savetxt(DATA_DIR / 'dx.txt', dx)
# np.savetxt(DATA_DIR / 'dp.txt', dp)
# np.savetxt(DATA_DIR / 'Euler100' / 'dx.txt', dx_s100)
# np.savetxt(DATA_DIR / 'Euler100' / 'dp.txt', dp_s100)
