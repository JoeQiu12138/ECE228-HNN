...
# Train the NN
def run_odeNet_HH_MM(X0, tf, neurons, epochs, n_train,lr, PATH= "models/model_HH_adaptive", loadWeights=False,
                     minLoss=1e-3):

    print(f"--- 确认：当前训练函数正在操作的文件路径是: {PATH} ---")
...
def trainModel(X0, t_max, neurons, epochs, n_train, lr,  loadWeights=True, minLoss=1e-6, showLoss=True, PATH ='models/model_HH'):
    model,loss,runTime, loss_erg = run_odeNet_HH_MM(X0, t_max, neurons, epochs, n_train,lr,  loadWeights=loadWeights, minLoss=minLoss, PATH = PATH)
...
def loadModel(PATH="models/model_HH_adaptive"):
    # 这一行是新加的，用来解决报错
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') 
    
    checkpoint = torch.load(PATH, map_location=device)
    model = odeNet_HH_MM(neurons) 
    model.load_state_dict(checkpoint['model_state_dict'])
    
    # 顺便在这里把 a 打印出来，看看它的庐山真面目！
    try:
        print(f">>> 验证成功！AdaptiveSin 的频率参数 a = {model.actF.a.item()}")
    except:
        pass
...
# ========================================================
# 步骤 1: SHORT TIME (热身阶段)
# 目标：让 AdaptiveSin 的参数 a 在稳定区域找到正确的物理频率
# ========================================================
print("\n>>> 开始第一阶段：SHORT TIME 训练 (6*pi)...")
t_max_short, N_short = 6 * np.pi, 500
neurons = 80
epochs_short = int(2e4) 
lr_short = 5e-3 # 稍微保守的学习率

# 路径：先存到一个临时热身模型中
PATH_SHORT = "models/model_HH_short"

# 执行训练：loadWeights=False (从零开始)
#trainModel(X0, t_max_short, neurons, epochs_short, N_short, lr_short, 
#           loadWeights=False, minLoss=1e-8, showLoss=True, PATH=PATH_SHORT)

# 验证一下第一阶段学到的 a
#model_temp = loadModel(PATH=PATH_SHORT)
#print(f"--- 第一阶段结束，当前频率 a = {model_temp.actF.a.item():.4f} ---")


# ========================================================
# 步骤 2: LONG TIME (冲刺阶段)
# 目标：加载第一阶段的“物理底子”，挑战 12*pi 的长周期预测
# ========================================================
print("\n>>> 开始第二阶段：LONG TIME 训练 (12*pi)...")
t_max_long, N_long = 12 * np.pi, 500
epochs_long = int(5e4)
lr_long = 1e-3 # 在长周期挑战中，使用更小的学习率来防止参数跑偏

# 路径：存到最终的自适应模型中
PATH_LONG = "models/model_HH_adaptive"

# 执行训练：loadWeights=True (!!! 关键：继承第一阶段的成果)
# 注意：你需要确保你的 trainModel 函数内部逻辑是加载 PATH_SHORT 的权重
trainModel(X0, t_max_long, neurons, epochs_long, N_long, lr_long, 
           loadWeights=True, minLoss=1e-8, showLoss=True,PATH=PATH_SHORT)
